...
def estimate_dim(data: np.ndarray, t: float | None = None, neighbors: int | None = None, kernel: Literal["numpy", "cp"] = "cp") -> np.ndarray:
    """
    ...
    match kernel:
        case "numpy":
            d = scipy.spatial.distance.cdist(data, data, 'sqeuclidean')
            d.sort(axis=1)
            d = d[:, 1:][:, :neighbors]
            if t is None:
                t = 1 / d.mean()
        ...
        case "cupy":
            d = cupyx.scipy.spatial.distance.cdist(data, data, 'sqeuclidean')
            d.sort(axis=1)
            d = d[:, 1:][:, :neighbors]
            if t is None:
                t = 1 / d.mean()
